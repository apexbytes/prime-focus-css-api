...
import { identityProviderRouter, ssoRouter } from '../modules/sso/index.js';
...
// -- identity & access --------------------------------------------------------
// Mounted before '/auth' so the more specific paths are matched first.
v1Router.use('/auth/devices', trustedDeviceRouter);
v1Router.use('/auth/sso', ssoRouter);
...
// Provider configuration, not the sign-in flow: `/auth/sso` is the flow, and
// the two have entirely different audiences.
v1Router.use('/identity-providers', identityProviderRouter);
